...
export const getLatestWaterLevel = async (req, res) => {
  try {
    ...
    const data = await WaterLevel.findOne({ camera_id })
      .sort({ createdAt: -1, timestamp: -1, _id: -1 })
      .lean();

    if (!data) {
      return res.status(404).json({
        message: "No water level data found",
        camera_id,
      });
    }
    ...
    res.status(500).json({
      message: "Failed to fetch latest water level",
      error: err.message,
    });
  }
};
...
export const getWaterLevelHistoryByCamera = async (req, res) => {
  try {
    ...
    const data = await WaterLevel.find({ camera_id })
      .sort({ timestamp: 1 })
      .lean();
    ...
    res.status(500).json({
      message: "Failed to fetch water level history",
      error: err.message,
    });
  }
};