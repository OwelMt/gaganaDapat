...
import ReliefRequestsHistory from "./ReliefRequestsHistory";
jest.mock("react-router-dom", () => { const navigate = jest.fn(); const location = { search: "", state: { mode: "edit", request: { _id: "r1", requestType: "foodpacks", disaster: "Flood", rows: [{ evacuationCenterName: "Hall", male: 0, female: 0 }] } } }; return { useNavigate: () => navigate, useLocation: () => location }; }, { virtual: true });
...
test("invalid Excel opens a correction modal without replacing current rows", async () => {
  const { container } = render(<ReliefRequestsHistory />);
  const male = await screen.findByRole("spinbutton", { name: "Hall Male" });
